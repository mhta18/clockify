...
class ProjetcViewTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.detail_url = reverse(
            "projects-detail", kwargs={"pk": self.project.uuid}
        )
    ...
    def test_delete_projects_return_success(self):
        ...
        self.client.force_authenticate(user=self.user)
        router_create_url = reverse("projects-list")
        create_response = self.client.post(router_create_url, data=payload, format="json")
        self.assertEqual(create_response.status_code, status.HTTP_201_CREATED)

        delete_response = self.client.delete(self.detail_url, data=payload, format="json")
